 ...
 *     the document and marked informational: SPDX 2.x has no SBOM type
 *     field at all, and a supplier-level SBOM legitimately references
...
export const AUTOMOTIVE_SBOM_PROFILE: ComplianceProfile = {
  ...
  description:
    ...
    'report is not a statement of contractual compliance. SBOM type cannot be ' +
    'expressed in SPDX 2.x (SPDX 3 sbomType and the CycloneDX lifecycle phase ' +
    'are read) and external document references are absent by design in a ' +
    ...
    'the file name is SPDX packageFileName, which CycloneDX has no field for. ' +
  ...
  checks: [
    ...
    { id: 'sbom-timestamp', type: 'document-field', field: 'created', label: 'SBOM timestamp' },
    {
      ...
      field: 'sbomType',
      informational: true,
      label: 'SBOM type (SPDX 3 sbomType or CycloneDX lifecycle; not expressible in SPDX 2.x)',
    },
    ...
    { id: 'component-file-name', type: 'package-coverage', field: 'fileName', label: 'File name (SPDX packageFileName)' },
