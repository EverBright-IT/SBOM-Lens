/**
 * Minimal deterministic ustar writer — the extension host packs registry
 * downloads into a CTF-shaped tar that the existing delivery walker reads
 * unchanged. Mirrors packages/core/scripts/tar-writer.mjs (fixtures): sorted
 * entries, zeroed mtime/uid/gid, PAX headers for long names, matching what
 * Go's archive/tar (the ocm CLI) emits.
 */
...
export function writeTar(entries: readonly TarWriteEntry[]): Uint8Array {
